...
if(mapElement.dataset.lon && mapElement.dataset.lat){
    setMapView(mapElement.dataset.lon, mapElement.dataset.lat,mapElement.dataset.message,mapElement.dataset.radius );
}
...
if(mapData){
    ...
    locs.forEach(locData => {
        ...
        mark.addTo(map)
        mark.bindPopup( makePopupContent(locData.annonce, locData.title, locData.photo));
    });
}
...
if(showBtns != null && showBtns.length > 0){
    ...
    showBtns.forEach(btn => {
        L.marker([btn.dataset.lat, btn.dataset.lon]).addTo(map)
...
function makePopupContent(id, title, photo) {
    ...
    let image = ((photo) 
        ...
        // let image= makeImage("/img/imgAnnonces/default.png", imgSize, "photo_annonce" );
